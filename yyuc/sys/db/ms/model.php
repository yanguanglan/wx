...
class Model extends SampleModel{
	...
	public function find($id=null,$pam=null){
		...
		if($id === null){
			...
			$fillok = $this->YYUCSYS_db->fill_one_model($this,$id,$pam);
		}else{
			$fillok = $this->YYUCSYS_db->fill_one_model($this,"id='$id'");	
		}
		...
		if(!isset(self::$YYUCSYS_FIELD_LABLE[$this->YYUCSYS_tablename])){
			...
			foreach ($defaults as $default){
				if(trim($default['Default'])!=''){
					//存储默认值
					self::$YYUCSYS_FIELD_DEFAULT[$this->YYUCSYS_tablename][$default['Field']] = str_replace("')",'', str_replace("('",'', $default['Default']));
				}
	...
	public function has($condition=null,$pam=null){
		if($condition===null){
			$sql = $this->YYUCSYS_db->com_sql($this->YYUCSYS_real_tablename,$this->YYUCSYS_condition,null,null,"id",$this->YYUCSYS_pam);
		}else{
			$sql = $this->YYUCSYS_db->com_sql($this->YYUCSYS_real_tablename,$condition,null,null,"id",$pam);
		}
		DBDes::init_by_model($this,'R');
		$res = $this->YYUCSYS_db->query($sql);
		return count($res)>0;	
