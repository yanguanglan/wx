...
class SampleModel{
	 ...
	 */
	function vercodeok(){
		return $this->YYUC_vercode == Session::get('YYUC_vercode');
	}
	/**
	 * 输出password标签
	 * @param string $name 字段名称 标签name
	...
	function datetime($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" value="'.$datevalue.'" '.$this->field_required_string($name).' id="'.$this->elid($name).'" '.$attrs.' onfocus="yyuccalendar.initCalendar(this,true,$(this).next(\'input\')[0],'.$isdate.')"/><input type="hidden" relobj="yyuccalendar" rel="yyuc" value="'.$datevalue.'" name="'.$this->elname($name).'" />';
		return $tag;
	}
	/**
	 * 输出日期标签
