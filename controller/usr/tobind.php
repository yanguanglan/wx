...
if($pub->has_id()){
	...
	if($step=='1'){
		...
		sleep(1);
		//修改开发路径
		$http->post('/cgi-bin/callbackprofile?t=ajax-response&lang=zh_CN&token='.$pub->token, array('url'=>'http://www.weiqimobile.com/mpapi.html?appid='.$pub->id,'callback_token'=>md5($pub->id.Conf::$management_center_target)));
		Log::error($http->getContent(true));
		sleep(1);
		//开启开发接口
