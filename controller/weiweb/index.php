...
if(Request::get('state')=='qqreg'){
	...
	$keysArr = array(
			...
			"client_id" => '100543439',
			"redirect_uri" => urlencode('http://www.www.weiqimobile.com'),
...
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'houtai.www.weiqimobile.com') !== false || strpos(strtolower($_SERVER ['HTTP_HOST']), 'wx.dxcx.com') !== false){
	Redirect::to('/agencyAdmin/login.html');
}
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'weixin.dxcx.com') !== false){
	$gkw = "微信管家、微信营销、微房产、微生活、微餐饮、微汽车、微网站、微商城、微喜帖、微营销、微信定制开发";
	$gde = "微信管家,最大的微信公众智能服务平台,管家十大微体系:微生活、微菜单、微网站、微会员、微活动、微商城、微服务、微房产、微汽车、微支付、微客服,企业微营销必备。";
	$gti = "微信管家--最大的微信公众营销推广平台--盛世源码团队_微信营销,如此简单！";
}else{
	$gkw = "微管家、微信营销、微信代运营、微信托管、微网站、微商城、微营销、微信定制开发";
	$gde = "微信管家,国内最大的微信公众智能服务平台,管家十大微体系:微菜单、微官网、微会员、微活动、微商城、微推送、微服务、微统计、微支付、微客服,企业微营销必备。";
	$gti = "微信管家--—国内最大的微信公众服务平台！";
}
...
$mu = new Model('user');
		...
		if(!empty($u->un) && !empty($u->pwd) && ($mu->is_real(array('un','pwd')) || $mu->is_real(array('email','pwd')) || $mu->is_real(array('telephone','pwd')))){			
			if($mu->isstop != '0'){
				...
				if(!$_SERVER['IS_OEM'] && (strpos(strtolower($_SERVER ['HTTP_HOST']),'www.www.weiqimobile.com') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'ww.com') === false || strpos(strtolower($_SERVER ['HTTP_HOST']),'wx.com') === false)){
...
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'www.weiqimobile.com') !== false){
	Response::write(file_get_contents(YYUC_FRAME_PATH.YYUC_PUB.'/static/wxgjcn/index.html'));
	die();
}
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'weiqi.cc.com') !== false){
	Response::write(file_get_contents(YYUC_FRAME_PATH.YYUC_PUB.'/static/wxgjcom/index.html'));
	die();
}
if(strpos(strtolower($_SERVER ['HTTP_HOST']),'weixin.dxcx.com') !== false){
	Response::write(file_get_contents(YYUC_FRAME_PATH.YYUC_PUB.'/static/wxdxcx/index.html'));
	die();
}