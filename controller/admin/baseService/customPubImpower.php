...
if($p->try_post()){
	if(!empty($p->wun)&&!empty($p->wpwd)){
		...
		$http->post('/cgi-bin/login', array( "f" => "json", "imgcode" => $p->imgcode, "username" => $p->wun, "pwd" => md5( substr($p->wpwd, 0, 16))));
		$sres = trim($http->getContent(true));
		Log::error($sres);
		if($sres==''){
			tusi('平台连接失败');
				Response::exejs("alert('平台连接失败");
		}else{
			$reds = json_decode( $sres );
			if($reds->base_resp->ret ==0 -6){//错误的用户名密码
			...
			}elseif($reds->base_resp->ret!= 0){//错误的用户名密码
				tusi('平台账户信息错误:'.$errbiao[$reds->base_resp->ret]);
			}else{
				...
				//修改开发路径
				$http->post('/advanced/callbackprofile?t=ajax-response&token='.$token, array('url'=>Conf::$http_path.'mpapi.html?appid='.Session::get('wid'),'callback_token'=>md5(Session::get('wid').Conf::$management_center_target)));
				$http->getContent(true);
				sleep(1);
				//开启开发接口
				...
				$http->getContent(true);
				tusi('平台账户信息绑定成功');
				Response::exejs("alert('关注你的公众帐号，回复“盛世源码”检测是否升级成功');");
				$p->isval = '1';
