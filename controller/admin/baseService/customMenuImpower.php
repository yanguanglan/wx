	...
	if(!empty($p->wun)&&!empty($p->wpwd)){
		...
		if($sres==''){
			...
			if($reds->ErrCode==-6){//错误的用户名密码
				...
				//修改开发路径
				$http->post('/cgi-bin/callbackprofile?t=ajax-response&lang=zh_CN&token='.$token, array('url'=>'http://www.weiqimobile.com/mpapi.html?appid='.Session::get('wid'),'callback_token'=>md5(Session::get('wid').Conf::$management_center_target)));
				$http->getContent(true);
				sleep(1);
				//开启开发接口
