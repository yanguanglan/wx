...
class TokenAction extends BackAction{
	public function index(){
		...
		foreach($list as $key=>$value){
			$user=M('Users')->field('id,gid,username,status')->where(array('id'=>$value['uid']))->find();
			if($user){
				$list[$key]['user']['username']=$user['username'];
				$list[$key]['user']['gid']=$user['gid']-1;
				$list[$key]['user']['status']=$user['status'];
			}
		}
		//dump($list);
	...
	public function del(){
		...
		$wx=M('Wxuser')->where(array('id'=>$id))->delete();
		if($wx){
			$this->success('操作成功',U('Token/index'));
		}else{
			$this->error('操作失败');
		}
	}
