...
class AttachmentAction extends UserAction
{
    ...
    public function files()
    {
        ...
        $background = array('view' => array('name' => '微站背景', 'height' => 100, 'files' => array('1.jpg', '2.jpg', '3.jpg', '4.jpg', '5.jpg', '6.jpg', '7.jpg', '8.jpg', '9.jpg')), 'newyear' => array('name' => '贺卡背景', 'height' => 200, 'files' => array('1.jpg', '2.jpg', '3.jpg', '4.jpg', '5.jpg', '6.jpg', '7.jpg', '8.jpg')));
    ...
    public function my()
    {
		...
		foreach($list as $k=>$v){
			if($v['uptype']=='local' || empty($v['uptype'])){
				$list[$k]['url']=C('site_url').$v['url'];
			}
		}
	...
	public function delmy()
    {
        $id=$this->_get('id','intval');
		$db = M('Files');
        $where = array('token' => $this->token,'id'=>$id);
		$file=$db->where($where)->find();
		if($db->where($where)->delete()){
			//删除图片
			if($file['uptype']=='local' || empty($file['uptype'])){
				@unlink($_SERVER['DOCUMENT_ROOT'].__ROOT__.$file['url']);
			}
			$this->success('操作成功',U(MODULE_NAME.'/my?type=my'));
		}else{
			$this->error('操作失败',U(MODULE_NAME.'/my?type=my'));
		}
    }
}
?>