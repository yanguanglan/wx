...
class KtvAction extends UserAction
{
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'type' => 'Ktv');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Estate_son')->where(array('token' => session('token'),'type' => 'Ktv'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
    ...
    public function reservation()
    {
        ...
        $t_company = M('Company');
        $token = session('token');
        $where =  array('token'=>$token,'isbranch'=>1,'shortname'=>'Ktv');
        ...
        if(IS_POST){
            ...
            if($check == null){     
                    ...
                    if($t_company->add($_POST)){                     
                        $this->success('添加成功',U('Ktv/aboutus',array('token'=>session('token'))));
                        exit;
                    }else{
             ...
             if($t_company->where($wh)->save($_POST)){ 
                    $this->success('修改成功',U('Ktv/aboutus',array('token'=>session('token'))));
                    exit;
                }else{
