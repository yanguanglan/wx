...
class CarAction extends UserAction{
    ...
    public function reservation_uinfo(){
		...
		//$user=M('Caruser')->field('car_userName as truename,brand_serise,car_no as carnum,user_tel,car_care_mileage as km')->where(array('token'=>session('token')))->order('id desc')->find();
    ...
    public function utility(){
        $utility = M('Car_utility')->order('id desc')->select();
        $this->assign('utility',$utility);
        if((int)$this->_get('isPost') == 1){
            $status  = $this->_get('status');
            $id = (int)$this->_get('id');
            $check = M('Car_utility')->where(array('id'=>$id))->find();
            if(!$check){
                $this->error('操作失败');
            }
            if($status == 'on'){
                M('Car_utility')->where(array('id'=>$id))->save(array('status'=>0));
                 $this->success('关闭成功',U('Car/utility',array('token'=>session('token'))));exit;
            }else{
                M('Car_utility')->where(array('id'=>$id))->save(array('status'=>1));
                $this->success('开启成功',U('Car/utility',array('token'=>session('token'))));exit;
