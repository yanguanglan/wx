...
class JiudianAction extends UserAction{
		...
		if(!strpos($token_open['queryname'],'Jiudian')){
		...
		$this->jiudian_setcin=M('jiudian_setcin');
		$this->token=session('token');
		$this->assign('token',$this->token);
		$this->assign('module','Jiudian');
		$this->type="Jiudian";
	}
	...
	public function index(){
		...
		$count      = $this->Jiudian_model->where($where)->count();
		$Page       = new Page($count,20);
		$show       = $Page->show();
		$data = $this->Jiudian_model->where($where)->order('id desc')->select();
	...
	public function add(){
		...
		if(IS_POST){	
			...
			if($id = $this->Jiudian_model->add($_POST)){
	...
	public function set(){
		...
		$checkdata = $this->Jiudian_model->where(array('id'=>$id))->find();
		...
		if(IS_POST){ 
			...
			$check=$this->Jiudian_model->where($where)->find();

			if($check==false)$this->error('非法操作');

			if($this->Jiudian_model->create()){
				...
				if($this->Jiudian_model->where($where)->save($_POST)){
				...
				$this->error($this->Jiudian_model->getError());
	...
	public function del(){
        ...
        if(IS_GET){                              
            ...
            $back=$this->Jiudian_model->where($where)->delete();

            if($back==true){

				$this->jiudian_order->where($wher)->delete();
	...
	public function infos(){
		$pid=$this->_get('pid');
		$check=$this->Jiudian_model->where(array('id'=>$pid))->find();
		...
		$count = $this->jiudian_order->where($where)->count();	
		$Page = new Page($count,20);
		$show = $Page->show();
	...
	public function delinfos(){
        ...
        if(IS_GET){                              
            $where=array('id'=>$id,'token'=>$this->token);
            $check=$this->jiudian_order->where($where)->find();
            if($check==false)   $this->error('非法操作');
            $back=$this->jiudian_order->where($where)->delete();
            if($back==true){
	...
	public function setType(){
        ...
        if(IS_GET){                              
			...
			if($this->jiudian_order->where($where)->setField($data)){
