...
class RecognitionAction extends UserAction{
    public function _initialize(){
        ...
        $diymen = M('Diymen_set')-> where($where) -> find();
        if(!$diymen['appid'] || !$diymen['appsecret']){
            $this -> error('只有填写过appid和appsecret且微信官方认证的高级服务号才能使用本功能', '?g=User&m=Diymen&a=index&token='. $diymen['token']);
        }
    }
