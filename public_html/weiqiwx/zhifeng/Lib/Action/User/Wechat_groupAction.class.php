...
class Wechat_groupAction extends UserAction{
    ...
    public function _initialize(){
        ...
        if (!$this -> thisWxUser['appid'] || !$this -> thisWxUser['appsecret']){
            $this -> error("请先设置AppID和AppSecret再使用本功能，谢谢", '?g=User&m=Diymen&a=index&token='. $this -> thisWxUser['token']);
        }
        /*if ($this -> thisWxUser['winxintype'] != 3){
            $this -> error('只有微信官方认证的高级服务号才能使用本功能', '?g=User&m=Diymen&a=index&token='. $this -> thisWxUser['token']);
        }*/
    }
