	...
	public function upload(){
		...
		if ($this->upload_type=='upyun'){
			...
			if (IS_POST){
				...
				echo '<script>location.href="'.C('site_url').'/index.php?g=User&m=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
			}else {
				$this->display('local');
			}
		}
		...
		if(isset($_GET['sign'])){ /// 正常签名
			if(md5("{$_GET['code']}&{$_GET['message']}&{$_GET['url']}&{$_GET['time']}&".$form_api_secret) == $_GET['sign']){
				...
				if($_GET['code'] == '200'){
					...
					M('Files')->add(array('token'=>$this->token,'size'=>intval($fileinfo['Content-Length']),'time'=>time(),'type'=>$fileinfo['Content-Type'],'url'=>$fileUrl,'uptype'=>'upyun'));
				}else{
					$handled=1;
					/// 上传失败
				}
