<?php
class MeirongAction extends BaseAction{
    public $token;
    public $wecha_id;
    public function _initialize() {
        ...
        $get_ids = M('Estate')->where(array('token'=>$this->token,'type' => 'Meirong'))->field('res_id,classify_id')->find();
    ...
    public function index(){
        ...
        $data = M("Estate");
        $this->token=$this->_get('token');
        $where = array('token'=>$this->token,'type' => 'Meirong');
    ...
    public function  newlist(){
        ...
        $t_img = M('Img');
        $where = array('id'=>$mid,'token'=>$token,'type' => 'Meirong');
    ...
    public function housetype(){
        ...
        $where = array('token'=>$this->_get('token'),'type' => 'Meirong');
    ...
    public function album(){
        ...
        $album = $t_album->where(array('token'=>$this->_get('token'),'type' => 'Meirong'))->field('id,poid')->select();
        foreach ($album as $k => $v) {
             $list_photo = $Photo->where(array('token'=>$this->_get('token'),'id'=>$v['poid'],'addtype' => 'Meirong'))->field('id')->find();
             $photolist[]=$photo_list->where(array('token'=>$this->_get('token'),'pid'=>$list_photo['id'],'addtype' => 'Meirong'))->select();
    ...
    public function show_album(){
        $t_housetype = M('Estate_housetype');
        $id = (int)$this->_get('id');
        $where = array('token'=>$this->_get('token'),'id'=>$id,'type' => 'Meirong');
    ...
    public function impress(){
       ...
       // $impress = $t_impress->where('token'=>$this->_get('token'),'is_show'=>1)->order('sort desc')->select(); 
         $impress = $t_impress->where(array('is_show=1','type' => 'Meirong'))->order('sort desc')->select(); 
        $this->assign('impress',$impress);

        $t_expert = M('Estate_expert'); 
        $expert = $t_expert->where(array('token'=>$this->_get('token'),'type' => 'Meirong'))->order('sort desc')->select();
    ...
    public function  aboutus(){
        $company = M('Company');
        $token=$this->_get('token');
        $about = $company->where(array('token'=>$token,'shortname'=>'Meirong'))->find();
