...
class VoteAction extends BaseAction{
	public function index(){
        ...
        $vote_item = M('Vote_item')->where('vid='. $vote['id'])->order('rank DESC')->select();        
	...
	public function add_vote(){	
        ...
        $where   = array('vid'=>$tid,'wecha_id'=>$wecha_id);  
        ...
        $data = array('item_id'=>$chid,'vid'=>$tid,'wecha_id'=>$wecha_id,'touch_time'=>time(),'touched'=>1);     
		$ok = $recdata->add($data);
        $map['id'] = array('in',$chid);
        $t_item = M('Vote_item');
        $t_item->where($map)->setInc('vcount');       
        $total = M('Vote_record')->where('vid='.$tid)->count('touched'); 
        $item_count = M('Vote_item')->where('vid='.$tid)->order('rank DESC')->select();        
