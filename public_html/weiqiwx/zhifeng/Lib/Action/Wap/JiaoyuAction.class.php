...
class JiaoyuAction extends BaseAction{
    ...
    public function _initialize() {
        ...
        $get_ids = M('Estate')->where(array('token'=>$this->token,'type' => 'Jiaoyu'))->field('res_id,classify_id')->find();
    ...
    public function index(){
        ...
        $data        = M("Estate");
        $this->token = $this->_get('token');
        $where       = array('token' => $this->token,'type' => 'Jiaoyu');
    ...
    public function housetype()    {
        ...
        $where = array('token'=>$this->_get('token'),'type' => 'Jiaoyu');
    ...
    public function album(){
        ...
        $album = $t_album->where(array('token'=>$this->_get('token'),'type' => 'Jiaoyu'))->field('id,poid')->select();
    ...
    public function show_album(){
        $t_housetype = M('Estate_housetype');
        $id          = (int) $this->_get('id');
        $where = array('token'=>$this->_get('token'),'id'=>$id,'type' => 'Jiaoyu');
    ...
    public function impress(){
        $t_impress = M('Estate_impress');
         ...
         $impress = $t_impress->where(array('is_show=1','type' => 'Jiaoyu'))->order('sort desc')->select(); 
    ...
    public function aboutus(){
        $company = M('Company');
        $token=$this->_get('token');
        $about = $company->where(array('token'=>$token,'shortname'=>'Jiaoyu'))->find();
