...
class ProductAction extends BaseAction{
	...
	public function _initialize() {
		...
		$this->product_du=M('Product_weiqi');
	...
	public function product(){
		...
		$branchStoreCount=$company_model->where(array('token'=>$this->token))->count();
	...
	public function orderCart(){
		...
		if (IS_POST){
			...
			if ($pay_code){
				$this->redirect(U('Pay/'.$pay_code,array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'orderid'=>$orderid)));
			}else {
				$this->redirect(U('Product/my',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'success'=>1)));
			}
		}
	...
	public function index(){
		$product_du=M('product_weiqi');
		$thisdu=$this->product_du->where(array('token'=>intval($_GET['token'])))->find();
		$this->assign('thisdu',$thisdu);
	...
	public function email_body(){
		...
		if ($orders){
			$thisOrder=$orders[0];
			//订单信息
			$str="订单类型：购物<br>订单编号：".$thisOrder['orderid']."<br>姓名：".$thisOrder['truename']."\r\n电话：".$thisOrder['tel']."\r\n地址：".$thisOrder['address']."\r\n下单时间：".date('Y-m-d H:i:s',$thisOrder['time'])."<br>";
			...
			if ($products){
				...
				foreach ($products as $p){
					...
					$str .=$p['name']."  ".$products[$i]['count']."件  单价：".$p['price']."元\r\n";
					$i++;
				}
			}
