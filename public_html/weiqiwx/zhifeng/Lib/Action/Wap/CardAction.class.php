...
class CardAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        if ($cardsCount && isset($_GET['mycard'])) {
            echo '<script>location.href="'.U('Card/card',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$cards[0]['cardid'])).'";</script>';
        }
            ...
            if (!$hasApplied) {
                ...
                if (!$card) {
                    ...
                    if (intval($thisCard['miniscore']) == 0 || $userScore > intval($thisCard['miniscore'])) {
                        $error = -4;
                        header('Location:'.U('Userinfo/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$thisCard['id']))); 
                    } else {
                        $error = -3;
                    }
                }
	...
	public function accountlogs(){
        ...
        if ($token != false&&$wecha_id != false){
			...
			$firstSecondOfMonth = mktime(0, 0, 0, $month, 1, $year);
			$lastSecondOfMonth  = mktime(23, 59, 59, $month, 30, $year);
			$accountlogs= M('Member_accountlog')->where(array('token'=>$token,'wecha_id'=>$wecha_id,'time'=>array('lt',$lastSecondOfMonth),'time'=>array('gt',$firstSecondOfMonth)))->select();
			$this->assign('accountlogs',$accountlogs);		  
        } else {
            $this->error('无此信息');
        }
        $this->display();
