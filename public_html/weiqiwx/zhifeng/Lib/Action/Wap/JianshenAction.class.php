...
class JianshenAction extends BaseAction{
    ...
    public function _initialize() {
        ...
        $get_ids = M('Estate')->where(array('token'=>$this->token,'type' => 'Jianshen'))->field('res_id,classify_id')->find();
    ...
    public function index(){
        ...
        $data        = M("Estate");
        $this->token = $this->_get('token');
        $where       = array('token' => $this->token,'type' => 'Jianshen');
    ...
    public function housetype()    {
        ...
        $where = array('token'=>$this->_get('token'),'type' => 'Jianshen');
    ...
    public function album(){
        ...
        $album      = $t_album->where(array('token' => $this->_get('token'),'type' => 'Jianshen'  ))->field('id,poid')->select();
        foreach ($album as $k => $v) {
            $list_photo  = $Photo->where(array('token' => $this->_get('token'),'type' => 'Jianshen','id' => $v['poid']))->field('id')->find(); 
    ...
    public function show_album(){
        $t_housetype = M('Estate_housetype');
        $id          = (int) $this->_get('id');
        $where       = array('token' => $this->_get('token'),'id' => $id,'type' => 'Jianshen');
    ...
    public function impress(){
        $t_impress = M('Estate_impress');
         ...
         $impress = $t_impress->where(array('is_show=1','type' => 'Jianshen'))->order('sort desc')->select(); 
    ...
    public function  aboutus(){
        $company = M('Company');
        $token   = $this->_get('token');
        $about   = $company->where(array( 'token' => $token,'shortname'=>'Jianshen' ))->find();
