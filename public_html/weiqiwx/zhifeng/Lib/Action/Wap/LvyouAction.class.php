...
class LvyouAction extends BaseAction{
    ...
    public function _initialize() {
        ...
        $get_ids = M('Estate')->where(array('token'=>$this->token,'type' => 'lvyou'))->field('res_id,classify_id')->find();
    ...
    public function index(){
        ...
        $data = M("Estate");
        $this->token=$this->_get('token');
        $where = array('token'=>$this->token,'type' => 'lvyou');
    ...
    public function housetype()    {
        ...
        $where = array('token'=>$this->_get('token'),'type' => 'lvyou');
    ...
    public function album(){
        ...
        $album = $t_album->where(array('token'=>$this->_get('token'),'type' => 'lvyou'))->field('id,poid')->select();
    ...
    public function show_album(){
        $t_housetype = M('Estate_housetype');
        $id = (int)$this->_get('id');
        $where = array('token'=>$this->_get('token'),'id'=>$id,'type' => 'lvyou');
    ...
    public function impress(){
       ...
       // $impress = $t_impress->where('token'=>$this->_get('token'),'is_show'=>1)->order('sort desc')->select(); 
         $impress = $t_impress->where(array('is_show=1','type' => 'lvyou'))->order('sort desc')->select(); 
        $this->assign('impress',$impress);

        $t_expert = M('Estate_expert'); 
        $expert = $t_expert->where(array('token'=>$this->_get('token'),'type' => 'lvyou'))->order('sort desc')->select();
    ...
    public function aboutus(){
        $company = M('Company');
        $token=$this->_get('token');
        $about = $company->where(array('token'=>$token,'shortname'=>'lvyou'))->find();
