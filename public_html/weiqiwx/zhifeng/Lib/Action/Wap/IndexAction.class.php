...
class IndexAction extends BaseAction{
	...
	public function _initialize(){
		...
		$info2=M('Classify')->where(array('token'=>$this->_get('token'),'status'=>1,'fid'=>array('gt',0)))->order('sorts desc')->select();
	...
	public function index(){
        ...
        $new=M('classify')->where(array('fid'=>$this->_get('classid'),'status'=>1,'token'=>$this->token))->select();
		$new=$this->convertLinks($new);//加外链等信息
	...
	public function lists(){
		...
		if($p==false){$p=0;}
		$res=$db->where($where)->order('sorts ASC,uptatetime DESC')->limit("{$p},".$pageSize)->select();
		...
		if($new)
		$this->assign('info',$this->convertLinks($new));

        $ifone=M("classify")->where(array("fid"=>$this->_get("classid")))->select();
		if($ifone)
		$this->redirect(U('Index/index',array('classid'=>$this->_get('classid'),'token'=>$this->token,'tpl'=>$l['tpl'])));
	...
	public function getLink($url){
		...
		if (strExists($url,'刮刮卡')){
			...
			/*if(!strpos($url,'wecha_id')){
				if (strpos($url,'?')){
					$link=$url.'&wecha_id='.$this->wecha_id;
				}else {
					$link=$url.'?wecha_id='.$this->wecha_id;
				}
			}else{
