...
class WxPayHelper
{
	...
	*/
	...
	//生成发货通知
	function create_deliver_package($orderinfo){
		 try {
			...
			$obj['openid']              = $orderinfo['openid'];
			$obj['transid']             = $orderinfo['transid'];
			$obj['out_trade_no']        = $orderinfo['out_trade_no'];
			$obj['deliver_timestamp']   = $deliver_timestamp;
			$obj['deliver_status']      = "1";
			$obj['deliver_msg']         = "ok";
			
			//get_biz_sign函数受保护，需要先取消一下，否则会报错
			$app_signature  = $this->get_biz_sign($obj);
